...
df = pd.read_csv('med_gen.csv', sep=';',
                 usecols=['DEPARTEMENT', 'DENSITE /100 000 hab.'],
                 converters={'DEPARTEMENT': clean}
                 )
print(df.dtypes)
print(df)

maxi = df['DENSITE /100 000 hab.'].max()
print(maxi)
...
for record, shape in zip(records, shapes):
    ...
    print(dep['DENSITE /100 000 hab.'])
    dens = 0
    colorVal = scalarMap.to_rgba(dep['DENSITE /100 000 hab.'])
